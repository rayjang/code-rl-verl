...
def main():
    ...
    with pd.ExcelWriter(OUT, engine="openpyxl") as w:
        experiments.to_excel(w, "Experiments", index=False)
        pd.DataFrame(curves).to_excel(w, "TrainingCurves", index=False)
        pd.DataFrame(rubric_rows).to_excel(w, "RubricVersions", index=False)
        pd.DataFrame(verifier_rows).to_excel(w, "VerifierVersions", index=False)
        pd.DataFrame(reward_rows).to_excel(w, "RewardVersions", index=False)
        ablation.to_excel(w, "Ablations", index=False)
        (pd.concat(audit) if audit else pd.DataFrame()).to_excel(w, "InstanceResults", index=False)
        (pd.concat(audit)[lambda d: d["status"] != "ok"] if audit else pd.DataFrame()).to_excel(w, "DatasetAudit", index=False)
        difficulty.to_excel(w, "Difficulty", index=False)
        pd.DataFrame(env_rows).to_excel(w, "EnvironmentManifest", index=False)
        final.to_excel(w, "FinalSelection", index=False)
