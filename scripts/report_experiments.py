...
def main():
    ...
    for i, r in reg.items():
        s = r.get("summary") or {}
        v = val_keys(s)
        pick = lambda sub: next((round(v[k], 4) for k in v if sub in k), None)
        rows.append({"id": i, "parent": r.get("parent"), "stage": r.get("stage"), "reward": r.get("reward_version"), "verifier": r.get("verifier_version"),
                     ...
                     "val_score": pick("score/mean"), "val_resolved": pick("rule_correctness"), "val_f2p": pick("f2p_frac"), "val_p2p": pick("p2p_frac"),
                     "val_apply": pick("patch_apply"), "val_format": pick("patch_format"), "kl": None if s.get("final_kl") is None else round(s["final_kl"], 5),
    ...
    cols = ["id", "stage", "reward", "verifier", "rubric", "steps", "train_reward", "val_score", "val_resolved", "val_f2p", "val_p2p", "val_apply", "kl", "entropy", "max_grad", "decision"]
